...
def psvariance(R, growthf=1,low = -5.0,high=3.0,num=10000):
    """
...
def f_of_sigma(sigma,A=0.186,a=1.47,b=2.57,c=1.19,z=0,Delta=200):
    ...
    A = A*(1+z)**(-0.14)
    a = a*(1+z)**(-0.06)
    alpha = 10**(-(0.075/np.log10(Delta/75))**1.2)
    b = b*(1+z)**(-alpha)
    f = A*((sigma/b)**(-a)+1)*np.exp(-c/sigma/sigma)
    return f

def dlogsigma_dr(R,eps=None):
    """
    ...
    **kwargs: optional
    ...
    #No redshift dependence oon the slope
    lns = lambda x: 0.5*np.log(psvariance(x))
